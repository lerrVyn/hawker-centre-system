// Lervyn Ang
// Covers SA-37 (Submit Feedback), SA-208 (View feedback for a stall),
// SA-209 (Edit/Delete own feedback). Owner replies live in feedbackReplies.js
// so this file stays focused on the customer-facing feedback resource.
...
/**
 * SA-37: CREATE - submit feedback for a stall (protected)
 */
...
/**
 * SA-208: READ - all feedback for a stall (public), newest first, with the
 * stall's average rating and each feedback's owner reply (if any).
 * Handles the empty state by returning an empty array + average: null
 * instead of a 404, since "no reviews yet" is a valid state, not an error.
 */
router.get("/stall/:stall_id", async (req, res) => {
  ...
  try {
    ...
    const feedbackResult = await pool
      ...
      .query(
        `SELECT f.feedback_id, c.name AS customer_name, f.rating, f.comment, f.created_at,
         ...
         LEFT JOIN feedback_replies r ON r.feedback_id = f.feedback_id
    ...
    const avgResult = await pool
      ...
      .input("stall_id", sql.Int, stall_id)
      .query(
        `SELECT AVG(CAST(rating AS FLOAT)) AS average_rating, COUNT(*) AS total_reviews
         FROM feedback WHERE stall_id = @stall_id`
      );

    const { average_rating, total_reviews } = avgResult.recordset[0];

    res.json({
      stall_id,
      average_rating: average_rating !== null ? Math.round(average_rating * 10) / 10 : null,
      total_reviews,
...
/**
 * READ - single feedback entry by id (public)
 */
...
/**
 * SA-209: UPDATE - edit own feedback (protected, owner-only)
 */
...
/**
 * SA-209: DELETE - remove own feedback (protected, owner-only)
 */
router.delete("/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Owner replies FK-reference this feedback row, clear it first
    await pool
      .request()
      .input("feedback_id", sql.Int, feedback_id)
